...
export function EditorialFeatureCard({ post, href, label = 'Cover story' }: { post: SitePost; href: string; label?: string }) {
  return (
    <Link href={href} className="group block min-w-0 overflow-hidden rounded-[1.2rem] bg-[#eef1f4] text-[var(--slot4-page-text)]">
      <div className="relative aspect-[16/10] min-h-[430px] overflow-hidden">
        ...
        <div className="absolute inset-0 bg-[linear-gradient(180deg,rgba(40,42,49,.05),rgba(40,42,49,.7))]" />
        <div className="absolute inset-x-0 bottom-0 p-6 sm:p-9">
          <span className="rounded-full bg-white px-4 py-2 text-[10px] font-black uppercase tracking-[.18em] text-[var(--slot4-accent)]">{label}</span>
...
export function RailPostCard({ post, href, index }: { post: SitePost; href: string; index: number }) {
  return (
    <Link href={href} className={`group ${dc.layout.minRailCard} block overflow-hidden rounded-[1.1rem] bg-white shadow-[0_10px_34px_rgba(40,42,49,.08)] ${dc.motion.lift}`}>
      ...
      <div className="p-4">
        ...
        <h3 className="mt-3 line-clamp-3 text-xl font-black leading-[1.08] tracking-[-.035em]">{post.title}</h3>
      </div>
    </Link>
  )
}
...
export function CompactIndexCard({ post, href, index }: { post: SitePost; href: string; index: number }) {
  return (
    <Link href={href} className="group grid min-w-0 grid-cols-[46px_1fr] gap-4 border-t border-[#c7d0dc] py-5 first:border-t-0">
...
export function ArticleListCard({ post, href, index }: { post: SitePost; href: string; index: number }) {
  return (
    <Link href={href} className="group grid min-w-0 rounded-[1.1rem] border border-[#d7dde5] bg-white p-4 transition hover:-translate-y-1 hover:shadow-[0_18px_48px_rgba(40,42,49,.10)] sm:grid-cols-[240px_minmax(0,1fr)] sm:gap-7">
      <div className="relative aspect-[16/10] overflow-hidden rounded-[.9rem] bg-[var(--slot4-media-bg)]">
        <img src={getEditablePostImage(post)} alt={post.title} className="absolute inset-0 h-full w-full object-cover transition duration-500 group-hover:scale-105" />
      </div>
