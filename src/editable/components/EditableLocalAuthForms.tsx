...
const inputClass = 'h-[3.25rem] rounded-full border border-[#c7d0dc] bg-[#f6f8fb] px-5 text-base font-bold text-[var(--slot4-page-text)] outline-none transition placeholder:text-[var(--slot4-soft-muted-text)] focus:border-[var(--slot4-accent)]'
const buttonClass = 'inline-flex h-[3.25rem] items-center justify-center rounded-full bg-[var(--slot4-dark-bg)] px-6 text-sm font-black text-white transition hover:bg-[var(--slot4-accent)] disabled:opacity-60'
...
export function EditableLocalLoginForm() {
  ...
  const submit = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (user && user.password !== password) {
      setStatus('error')
      setMessage('The password does not match this account.')
      return
    }
    const nextUser: LocalUser = user || {
      name: normalizedEmail.split('@')[0] || 'Member',
      email: normalizedEmail,
      password,
      createdAt: new Date().toISOString(),
    }
    if (!user) saveUsers([nextUser, ...users])
