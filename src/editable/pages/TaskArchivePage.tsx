    ...
    <EditableSiteShell>
      <main className="min-h-screen bg-white text-[var(--slot4-page-text)]">
        <section className="bg-white">
          <div className="mx-auto flex max-w-[1320px] flex-col gap-8 px-4 py-14 sm:px-6 lg:flex-row lg:items-end lg:justify-between lg:px-8 lg:py-20">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.24em] text-[var(--slot4-accent)]">Resources</p>
              <h1 className="mt-3 max-w-4xl text-5xl font-black leading-[1.05] tracking-[-0.055em] sm:text-7xl">
                {category === 'all' ? label : categoryLabel}
              </h1>
            </div>
            <form action={basePath} className="flex rounded-full border border-[#c7d0dc] bg-[#f6f8fb] p-1">
              <select name="category" defaultValue={category} className="h-12 min-w-44 bg-transparent px-4 text-sm font-black outline-none">
                <option value="all">All categories</option>
                {categories.map((item) => <option key={item.slug} value={item.slug}>{item.name}</option>)}
              </select>
              <button className="rounded-full bg-[var(--slot4-dark-bg)] px-6 text-sm font-black text-white">Filter</button>
            </form>
          </div>
        </section>

        <section className="bg-white">
          <div className="mx-auto flex max-w-[1320px] gap-3 overflow-x-auto px-4 pb-8 text-sm font-black sm:px-6 lg:px-8">
            <Link href={basePath} className={category === 'all' ? 'rounded-full bg-[var(--slot4-dark-bg)] px-4 py-2 text-white' : 'rounded-full bg-[#eef1f4] px-4 py-2 hover:text-[var(--slot4-accent)]'}>Latest</Link>
            {categories.slice(0, 8).map((item) => (
              <Link key={item.slug} href={pageHref(basePath, item.slug, 1)} className={category === item.slug ? 'whitespace-nowrap rounded-full bg-[var(--slot4-dark-bg)] px-4 py-2 text-white' : 'whitespace-nowrap rounded-full bg-[#eef1f4] px-4 py-2 hover:text-[var(--slot4-accent)]'}>
                {item.name}
              </Link>
            ))}
          </div>
        ...
        {lead ? (
          <section className="mx-auto grid max-w-[1320px] gap-8 px-4 sm:px-6 lg:grid-cols-[1.12fr_.88fr] lg:px-8">
            <Link href={`${basePath}/${lead.slug}`} className="group relative min-h-[34rem] overflow-hidden rounded-[1.2rem] bg-[#eef1f4]">
              ...
              <div className="absolute inset-x-0 bottom-0 p-6 text-white sm:p-9">
                <span className="rounded-full bg-white px-4 py-2 text-[10px] font-black uppercase tracking-[0.2em] text-[var(--slot4-accent)]">{getCategory(lead, label)}</span>
            ...
            <div className="grid gap-5">
              <div className="rounded-[1.2rem] bg-[var(--slot4-accent)] p-7 text-white">
                <p className="text-xs font-black uppercase tracking-[0.24em]">Top stories</p>
                <p className="mt-3 text-3xl font-black leading-tight tracking-[-0.04em]">Media updates organized for fast discovery.</p>
              </div>
              {secondary.map((post, index) => (
                <Link key={post.id || post.slug} href={`${basePath}/${post.slug}`} className="group grid grid-cols-[8rem_1fr] overflow-hidden rounded-[1.1rem] border border-[#d7dde5] bg-white transition hover:-translate-y-1 hover:shadow-[0_18px_48px_rgba(40,42,49,.10)]">
                  ...
                  <div className="p-5">
                    ...
                    <h3 className="mt-3 text-xl font-black leading-tight tracking-[-0.035em]">{post.title}</h3>
                  </div>
                </Link>
              ))}
            </div>
        ...
        <section className="mx-auto max-w-[1320px] bg-white px-4 py-16 sm:px-6 lg:px-8 lg:py-20">
          <div className="mb-8 flex flex-wrap items-end justify-between gap-5 border-t border-[#c7d0dc] pt-10">
            <h2 className="text-4xl font-black tracking-[-0.04em] sm:text-5xl">More from the desk</h2>
            <p className="text-sm font-bold text-[var(--slot4-muted-text)]">Showing: {categoryLabel}</p>
          </div>

          {remaining.length ? (
            <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
              {remaining.map((post, index) => (
                <Link key={post.id || post.slug} href={`${basePath}/${post.slug}`} className="group overflow-hidden rounded-[1.1rem] border border-[#d7dde5] bg-white transition hover:-translate-y-1 hover:shadow-[0_18px_48px_rgba(40,42,49,.10)]">
                  <div className="aspect-[16/10] overflow-hidden bg-[#eef1f4]">
                  ...
                  <div className="p-5">
                    ...
                    <h3 className="mt-4 text-2xl font-black leading-[1.05] tracking-[-0.035em]">{post.title}</h3>
            ...
            <div className="rounded-[1.2rem] border border-dashed border-[#c7d0dc] bg-[#f6f8fb] p-12 text-center">
              <Search className="mx-auto h-8 w-8" />
              <h2 className="mt-4 text-3xl font-black">No stories found</h2>
              <p className="mt-2 text-sm text-[var(--slot4-muted-text)]">Try another category or publish a new media update.</p>
            </div>
          ) : null}

          <div className="mt-10 flex items-center justify-center gap-3">
            {pagination.hasPrevPage ? <Link href={pageHref(basePath, category, page - 1)} className="rounded-full border border-[#c7d0dc] bg-white px-5 py-3 text-sm font-black">Previous</Link> : null}
            <span className="rounded-full bg-[var(--slot4-dark-bg)] px-5 py-3 text-sm font-black text-white">Page {page} / {pagination.totalPages || 1}</span>
            {pagination.hasNextPage ? <Link href={pageHref(basePath, category, page + 1)} className="rounded-full border border-[#c7d0dc] bg-white px-5 py-3 text-sm font-black">Next</Link> : null}
          </div>
        </section>
      </main>
    </EditableSiteShell>
