    ...
    <section className="bg-white">
      <header>
        <div className="mx-auto max-w-[1180px] px-4 py-10 sm:px-6 lg:px-8 lg:py-16">
          <BackLink task={task} />
          <div className="mt-10 flex flex-wrap items-center justify-between gap-3 border-t border-[#c7d0dc] pt-6 text-[11px] font-black uppercase tracking-[0.16em]">
            <span className="rounded-full bg-[#d7e7ff] px-4 py-2 text-[var(--slot4-accent)]">{categoryOf(post, 'News')}</span>
            {published ? <time className="text-[var(--slot4-muted-text)]">{published}</time> : null}
          </div>
          <h1 className="mt-8 max-w-6xl text-5xl font-black leading-[1.02] tracking-[-0.055em] sm:text-6xl lg:text-[5.2rem]">{post.title}</h1>
      ...
      {images[0] ? (
        <figure className="mx-auto max-w-[1320px] px-4 sm:px-6 lg:px-8">
          <img src={images[0]} alt="" className="max-h-[760px] w-full rounded-[1.2rem] object-cover" />
          <figcaption className="px-2 py-3 text-xs italic text-[var(--slot4-muted-text)]">Featured image for {post.title}</figcaption>
        </figure>
      ) : null}

      <div className="mx-auto grid max-w-[1180px] gap-12 px-4 py-12 sm:px-6 lg:grid-cols-[minmax(0,760px)_300px] lg:px-8 lg:py-16">
        <article className="min-w-0 rounded-[1.2rem] border border-[#d7dde5] bg-white p-6 shadow-[0_16px_46px_rgba(40,42,49,.06)] sm:p-9">
...
function RelatedPanel({ task, post: _post, related, compact = false }: { task: TaskKey; post: SitePost; related: SitePost[]; compact?: boolean }) {
  ...
  return (
    <aside className="min-w-0 space-y-5">
      {!compact ? (
        <div className="rounded-[1.1rem] border border-[#d7dde5] bg-[#f6f8fb] p-5">
      ...
      {related.length ? (
        <div className="rounded-[1.1rem] border border-[#d7dde5] bg-white p-5">
...
function RelatedCard({ task, post }: { task: TaskKey; post: SitePost }) {
  ...
  return (
    <Link href={buildPostUrl(task, post.slug)} className="group flex gap-3 border-t border-[#d7dde5] py-3 transition hover:text-[var(--slot4-accent)]">
      {image && task !== 'sbm' ? <img src={image} alt="" className="h-20 w-20 shrink-0 rounded-xl object-cover" /> : <div className="flex h-20 w-20 shrink-0 items-center justify-center rounded-xl bg-[var(--slot4-dark-bg)] text-white"><FileText className="h-6 w-6" /></div>}
...
function EditableComments({ slug, comments }: { slug: string; comments: Array<{ id: string; name: string; comment: string; createdAt: string }> }) {
  return (
    <section className="mt-12 rounded-[1.1rem] border border-[#d7dde5] bg-[#f6f8fb] p-5">
      <div className="flex items-center gap-2 text-lg font-black"><MessageCircle className="h-5 w-5" /> Comments</div>
      <div className="mt-5 grid gap-3">
        {comments.slice(0, 5).map((comment) => (
          <div key={comment.id} className="border-t border-[#d7dde5] py-4">
